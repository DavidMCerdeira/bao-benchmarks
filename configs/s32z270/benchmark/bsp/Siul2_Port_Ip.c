...
#define SIUL2_PORT_IP_SW_PATCH_VERSION_C            1
...
#define Call_Siul2_Port_Ip_ConfigInternalResistor(base, pin, pullConfig) \
    OsIf_Trusted_Call3params(Siul2_Port_Ip_ConfigInternalResistor, (base), (pin), (pullConfig))
#define Call_Siul2_Port_Ip_ConfigOutputBuffer(base, pin, enable, mux) \
    OsIf_Trusted_Call4params(Siul2_Port_Ip_ConfigOutputBuffer, (base), (pin), (enable), (mux))
#define Call_Siul2_Port_Ip_ConfigInputBuffer(base, pin, enable, inputMuxReg, inputMux) \
    OsIf_Trusted_Call5params(Siul2_Port_Ip_ConfigInputBuffer, (base), (pin), (enable), \
        (inputMuxReg), (inputMux))
#define Call_Siul2_Port_Ip_ConfigPinDirection(base, pin, direction) \
    OsIf_Trusted_Call3params(Siul2_Port_Ip_ConfigPinDirection, (base), (pin), (direction))
#define Call_Siul2_Port_Ip_GetValueConfigRevertPin(base, pin) \
    OsIf_Trusted_Call_Return2param(Siul2_Port_Ip_GetValueConfigRevertPin, (base), (pin))
#define Call_Siul2_Port_Ip_GetValuePinConfiguration(base, config, pin) \
    OsIf_Trusted_Call3params(Siul2_Port_Ip_GetValuePinConfiguration, (base), (config), (pin))
#else
#define Call_Siul2_Port_Ip_PinInit(config) Siul2_Port_Ip_PinInit(config)
#define Call_Siul2_Port_Ip_ConfigInternalResistor(base, pin, pullConfig) \
    Siul2_Port_Ip_ConfigInternalResistor(base, pin, pullConfig)
#define Call_Siul2_Port_Ip_ConfigOutputBuffer(base, pin, enable, mux) \
    Siul2_Port_Ip_ConfigOutputBuffer(base, pin, enable, mux)
#define Call_Siul2_Port_Ip_ConfigInputBuffer(base, pin, enable, inputMuxReg, inputMux) \
    Siul2_Port_Ip_ConfigInputBuffer(base, pin, enable, inputMuxReg, inputMux)
#define Call_Siul2_Port_Ip_ConfigPinDirection(base, pin, direction) \
    Siul2_Port_Ip_ConfigPinDirection(base, pin, direction)
#define Call_Siul2_Port_Ip_GetValueConfigRevertPin(base, pin) \
    Siul2_Port_Ip_GetValueConfigRevertPin(base, pin)
#define Call_Siul2_Port_Ip_GetValuePinConfiguration(base, config, pin) \
    Siul2_Port_Ip_GetValuePinConfiguration(base, config, pin)
#endif
/*==================================================================================================
*                                         LOCAL CONSTANTS
==================================================================================================*/
...
static inline void Siul2_Port_Ip_GetIMCRInstance(const Siul2_Port_Ip_PinSettingsConfig* config,
    uint32* imcrRegIdx, SIUL2_Type** imcrBase, uint8 inputMuxIterator);
 ...
 */
static inline void
Siul2_Port_Ip_Siul2AE_GetIMCRInstance(const Siul2_Port_Ip_PinSettingsConfig* config,
    uint32* imcrRegIdx, SIUL2_AE_Type** imcrBase, uint8 inputMuxIterator);
 ...
 */
static inline void
Siul2_Port_Ip_WriteIMCRConfiguration(const Siul2_Port_Ip_PinSettingsConfig* config);
...
static inline void Siul2_Port_Ip_GetMSCRConfiguration(Siul2_Port_Ip_PinSettingsConfig* config,
    const Siul2_Port_Ip_PortType* const base, uint16 pin);
...
static inline uint8 Siul2_Port_Ip_GetSiulInstance(const Siul2_Port_Ip_PinSettingsConfig* config)
{
    ...
    if ((config->base == IP_SIUL2_0) || (config->base == SIUL2_0_PDAC0) ||
        (config->base == SIUL2_0_PDAC1) || (config->base == SIUL2_0_PDAC2) ||
        (config->base == SIUL2_0_PDAC3)) {
        siulInstance = 0U;
    }
    ...
    else if ((config->base == IP_SIUL2_1) || (config->base == SIUL2_1_PDAC0) ||
        (config->base == SIUL2_1_PDAC1) || (config->base == SIUL2_1_PDAC2) ||
        (config->base == SIUL2_1_PDAC3)) {
        siulInstance = 1U;
    }
    ...
    else if ((config->base == IP_SIUL2_3) || (config->base == SIUL2_3_PDAC0) ||
        (config->base == SIUL2_3_PDAC1) || (config->base == SIUL2_3_PDAC2) ||
        (config->base == SIUL2_3_PDAC3)) {
        siulInstance = 2U;
    }
    ...
    else if ((config->base == IP_SIUL2_4) || (config->base == SIUL2_4_PDAC0) ||
        (config->base == SIUL2_4_PDAC1) || (config->base == SIUL2_4_PDAC2) ||
        (config->base == SIUL2_4_PDAC3)) {
        siulInstance = 3U;
    }
    ...
    else if ((config->base == IP_SIUL2_5) || (config->base == SIUL2_5_PDAC0) ||
        (config->base == SIUL2_5_PDAC1) || (config->base == SIUL2_5_PDAC2) ||
        (config->base == SIUL2_5_PDAC3)) {
        siulInstance = 4U;
    }
...
void Siul2_Port_Ip_PinInit(const Siul2_Port_Ip_PinSettingsConfig* config)
{
    ...
    if ((PORT_MUX_AS_GPIO == config->mux) && (PORT_OUTPUT_BUFFER_ENABLED == config->outputBuffer)) {
        ...
        if (config->initValue != PORT_PIN_LEVEL_NOTCHANGED_U8) {
            ...
            PORT_WRITE8(SIUL2_GPDO_ADDR8(siulInstance, config->mscrPdacSlot,
                            (uint16)config->pinPortIdx),
                config->initValue);
        }
#else
        if (config->initValue != PORT_PIN_LEVEL_NOTCHANGED_U8) {
            /* Write the value into GPDO register */
            PORT_WRITE8(SIUL2_GPDO_ADDR8(siulInstance, (uint16)config->pinPortIdx),
                config->initValue);
        }
#endif /* STD_ON == PORT_VIRTWRAPPER_SUPPORT */
    }
...
static inline void
Siul2_Port_Ip_Siul2AE_GetIMCRInstance(const Siul2_Port_Ip_PinSettingsConfig* config,
    uint32* imcrRegIdx, SIUL2_AE_Type** imcrBaseAE, uint8 inputMuxIterator)
{
    /* Write to Input Mux register */
    *imcrRegIdx = config->inputMuxReg[inputMuxIterator];
...
Siul2_Port_Ip_WriteIMCRConfiguration(const Siul2_Port_Ip_PinSettingsConfig* config)
{
    ...
    if (PORT_INPUT_BUFFER_ENABLED == config->inputBuffer) {
        for (inputMuxIterator = 0U; inputMuxIterator < FEATURE_SIUL2_MAX_NUMBER_OF_INPUT_U8;
             inputMuxIterator++) {
            ...
            if (PORT_INPUT_MUX_NO_INIT != config->inputMux[inputMuxIterator]) {
                ...
                if ((config->baseAE) != NULL_PTR) {
                    Siul2_Port_Ip_Siul2AE_GetIMCRInstance(config, &imcrRegIdx, &imcrBaseAE,
                        inputMuxIterator);
                } else
#endif /* IP_SIUL2_AE_BASE */
                {
...
#ifdef IP_SIUL2_AE
                    if ((config->baseAE) != NULL_PTR) {
                        imcrBaseAE->IMCR[imcrRegIdx % SIUL2_MAX_NUM_OF_IMCR_REG] =
                            SIUL2_AE_IMCR_SSS(config->inputMux[inputMuxIterator]);
                    } else
#endif /* IP_SIUL2_AE */
                    {
                        imcrBase->IMCR[imcrRegIdx % SIUL2_MAX_NUM_OF_IMCR_REG] =
...
void Siul2_Port_Ip_ConfigInternalResistor(Siul2_Port_Ip_PortType* const base, uint16 pin,
 ...
 *END**************************************************************************/
void Siul2_Port_Ip_SetPullSel(Siul2_Port_Ip_PortType* const base, uint16 pin,
    Siul2_Port_Ip_PortPullConfig pullConfig)
{
...
void Siul2_Port_Ip_ConfigOutputBuffer(Siul2_Port_Ip_PortType* const base, uint16 pin,
 ...
 *END**************************************************************************/
void Siul2_Port_Ip_SetOutputBuffer(Siul2_Port_Ip_PortType* const base, uint16 pin, boolean enable,
...
void Siul2_Port_Ip_ConfigInputBuffer(Siul2_Port_Ip_PortType* const base, uint16 pin, boolean enable,
    uint32 inputMuxReg, Siul2_Port_Ip_PortInputMux inputMux)
{
    /* Dummy address */
...
#ifdef IP_SIUL2_AE_BASE
    if (base > (Siul2_Port_Ip_PortType*)IP_SIUL2_AE_BASE) {
        SIUL2_PORT_IP_DEV_ASSERT((boolean)(!((SIUL2_DIPORT_IMCR_AE_IDX_START_U32 <= inputMuxReg) &&
            (SIUL2_DIPORT_IMCR_AE_IDX_END_U32 >= inputMuxReg))));
    }
#endif
 ...
 *END**************************************************************************/
void Siul2_Port_Ip_SetInputBuffer(Siul2_Port_Ip_PortType* const base, uint16 pin, boolean enable,
    uint32 inputMuxReg, Siul2_Port_Ip_PortInputMux inputMux)
{
    SIUL2_PORT_IP_DEV_ASSERT((boolean)(pin < SIUL2_NUM_OF_PIN_PORT_U16));
...
void Siul2_Port_Ip_ConfigPinDirection(Siul2_Port_Ip_PortType* const base, uint16 pin,
 ...
 *END**************************************************************************/
void Siul2_Port_Ip_SetPinDirection(Siul2_Port_Ip_PortType* const base, uint16 pin,
    Siul2_Port_Ip_PortDirectionType direction)
{
    SIUL2_PORT_IP_DEV_ASSERT((boolean)(base != NULL_PTR));
    SIUL2_PORT_IP_DEV_ASSERT((boolean)(pin < SIUL2_NUM_OF_PIN_PORT_U16));
...
uint32 Siul2_Port_Ip_GetValueConfigRevertPin(const Siul2_Port_Ip_PortType* const base, uint16 pin)
{
    uint32 u32PinIdx;
    ...
    /* Get the PortID:  Subtract the address of the first mscr from the base parameter then divide
     * to 4 and 16 because there are 16 ports but each MSCR has 4 bytes */
    ...
    if (base > (Siul2_Port_Ip_PortType*)IP_SIUL2_AE_BASE) {
        portNumber =
            (Siul2_Port_Ip_CoreType)((Siul2_Port_Ip_CoreType)((Siul2_Port_Ip_CoreType)((Siul2_Port_Ip_CoreType)
                                                                                           base -
                                                                  (Siul2_Port_Ip_CoreType)
                                                                      SIUL2_AE_MSCR_BASE) >>
                                         2U) >>
                4U);
    } else {
    ...
    /* Get the MscrID: 16 is the maximum number of pins per port so the portNumber need to multiply
     * 16 */
 ...
 * Description   : This function configures the pin configuration with the values from the
    ...
    Siul2_Port_Ip_PinSettingsConfig* config, uint16 pin)
{
    ...
    /* Get the PortID:  Subtract the address of the first mscr from the base parameter then divide
     * to 4 and 16 because there are 16 ports and each MSCR has 4 bytes */
    ...
    for (u32PinIdx = 0UL; u32PinIdx < MaxPinConfigured; u32PinIdx++) {
        ...
        if (ConfigPtr[u32PinIdx].pinPortIdx == u32MscrId) {
            ...
            for (inputMuxIterator = 0U; inputMuxIterator < FEATURE_SIUL2_MAX_NUMBER_OF_INPUT_U8;
                 inputMuxIterator++) {
                config->inputMuxReg[inputMuxIterator] =
                    ConfigPtr[u32PinIdx].inputMuxReg[inputMuxIterator];
                config->inputMux[inputMuxIterator] =
                    ConfigPtr[u32PinIdx].inputMux[inputMuxIterator];
            }
