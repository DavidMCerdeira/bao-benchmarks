...
#define CLOCK_IP_GATE_SW_PATCH_VERSION_C            1
...
static void Clock_Ip_ClockUpdateGateClockControlEnableGprPctl(Clock_Ip_NameType ClockName,
    boolean Gate);
#endif
...
static void Clock_Ip_ClockSetGateClockControlEnableGprPctl(Clock_Ip_GateConfigType const* Config)
{
    ...
    if (NULL_PTR != Config) {
        GateInformation =
            &Clock_Ip_axGateInfo[Clock_Ip_au8ClockFeatures[Config->Name][CLOCK_IP_GATE_INDEX]];
        GroupIndex = GateInformation->GroupIndex;
        GateIndex = GateInformation->GateIndex;
        GateBitField = GateInformation->GateBitField;

        if (Config->Enable != 0U) {
            Clock_Ip_apxGprClockControlEnable[GroupIndex]->PCTL[GateIndex] |=
                ((uint32)GPR_PCTL_MASK << GateBitField);
        } else {
            Clock_Ip_apxGprClockControlEnable[GroupIndex]->PCTL[GateIndex] &=
                ~((uint32)GPR_PCTL_MASK << GateBitField);
        }
