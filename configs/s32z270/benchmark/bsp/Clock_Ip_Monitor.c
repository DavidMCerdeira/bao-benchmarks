...
#define CLOCK_IP_MULTIPLIER_REFERENCE_COUNT_FORMULA1 \
    CLOCK_IP_FEATURE_MULTIPLIER_REFERENCE_COUNT_FORMULA1
#else
...
#define CLOCK_IP_MULTIPLIER_REFERENCE_COUNT_FORMULA2 \
    CLOCK_IP_FEATURE_MULTIPLIER_REFERENCE_COUNT_FORMULA2
#else
    ...
    if ((NULL_PTR != Config) && (Config->Enable != 0U)) {
        ...
        if (HashCmu[Index] != (((uint32)Config->MonitoredClockFrequency) ^ ((uint32)Config->Name))) {
            ...
            if ((ReferenceClk != 0U) && (BusClk != 0U) && (MonitoredClk != 0U)) {
                ...
                Cmp1 = CLOCK_IP_OFFSET_REFERENCE_COUNT_FORMULA1 +
                    (uint32)((CLOCK_IP_MULTIPLIER_REFERENCE_COUNT_FORMULA1 * ReferenceClk) / BusClk);
                /* Cmp2 = ceiling of (8 + (5 * fRef / fMonitor)) */
                Cmp2 = CLOCK_IP_OFFSET_REFERENCE_COUNT_FORMULA2 +
                    (uint32)((CLOCK_IP_MULTIPLIER_REFERENCE_COUNT_FORMULA2 * ReferenceClk) /
                        MonitoredClk);

                /* REF count = Max(cmp1,cmp2) */
                RefCount = (Cmp1 > Cmp2) ? Cmp1 : Cmp2;
                RefCount = (CLOCK_IP_CMU_REFERENCE_COUNTER_MINIMUM_VALUE_MULTIPLIER > RefCount) ?
                    CLOCK_IP_CMU_REFERENCE_COUNTER_MINIMUM_VALUE_MULTIPLIER :
                    RefCount;

                /* HTCR[HFREF] is ((fMonitoredClk mul_by (1000U plus
                 * CLOCK_IP_CMU_MONITORED_CLOCK_VARIATION)) divide_by (fReferenceClk mul_by (1000U
                 * minus CLOCK_IP_CMU_REFERENCE_CLOCK_VARIATION)) * RefCount) plus
                 * CLOCK_IP_CMU_FC_VAR plus 1U */
                ...
                /* LTCR[LFREF] is ((fMonitoredClk mul_by (1000U minus
                 * CLOCK_IP_CMU_MONITORED_CLOCK_VARIATION)) divide_by (fReferenceClk mul_by (1000U
                 * plus CLOCK_IP_CMU_REFERENCE_CLOCK_VARIATION)) * RefCount) minus
                 * CLOCK_IP_CMU_FC_VAR */
