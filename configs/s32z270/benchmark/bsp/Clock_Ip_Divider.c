...
#define CLOCK_IP_DIVIDER_SW_PATCH_VERSION_C            1
...
static void
Clock_Ip_SetCgmXDeDivStatWithoutPhaseWithTrigger(Clock_Ip_DividerConfigType const* Config);
#endif
...
static void Clock_Ip_SetCgmXDeDivStatWithoutPhase(Clock_Ip_DividerConfigType const* Config)
{
    ...
    if (NULL_PTR != Config) {
        ...
        do {
            DividerStatus = (Clock_Ip_apxCgm[Instance][SelectorIndex]->MUX_DIV_UPD_STAT &
                MC_CGM_MUX_DIV_UPD_STAT_DIV_STAT_MASK);
            TimeoutOccurred = Clock_Ip_TimeoutExpired(&StartTime, &ElapsedTime, TimeoutTicks);
        } while ((MC_CGM_MUX_DIV_UPD_STAT_DIV_STAT_PENDING == DividerStatus) &&
...
static void Clock_Ip_SetCgmXDeDivFmtStatWithoutPhase(Clock_Ip_DividerConfigType const* Config)
{
    ...
    if (NULL_PTR != Config) {
        ...
        if (Config->Value != 0U) {
            ...
            RegValue &= ~MC_CGM_MUX_2_DC_2_DIV_FMT_MASK;
            RegValue |= (((uint32)(Config->Options[0]) << MC_CGM_MUX_2_DC_2_DIV_FMT_SHIFT) &
...
Clock_Ip_SetCgmXDeDivStatWithoutPhaseWithTrigger(Clock_Ip_DividerConfigType const* Config)
{
    ...
    if (NULL_PTR != Config) {
        ...
        if ((Clock_Ip_apxCgm[Instance][SelectorIndex]->MUX_DIV_TRIG_CTRL &
                MC_CGM_MUX_DIV_TRIG_CTRL_TCTL_MASK) !=
            MC_CGM_MUX_DIV_TRIG_CTRL_COMMON_TRIGGER_DIVIDER_UPDATE) {
...
static void Clock_Ip_SetPlldigPll0divDeDivOutput(Clock_Ip_DividerConfigType const* Config)
{
    ...
    if (NULL_PTR != Config) {
        ...
        CLOCK_IP_DEV_ASSERT(!(Clock_Ip_apxPll[Instance].PllInstance->PLLODIV[DividerIndex] &
            PLLDIG_PLLODIV_DE_MASK));
