...
static void Clock_Ip_ResetCgmXCscCssClkswSwip(Clock_Ip_SelectorConfigType const* Config)
{
    ...
    if (NULL_PTR != Config) {
        ...
        SelectorMask =
            Clock_Ip_axFeatureExtensions[Clock_Ip_au8ClockFeatures[Config->Name]
                                                                  [CLOCK_IP_EXTENSION_INDEX]]
                .SelectorValueMask;
...
static void Clock_Ip_SetCgmXCscCssClkswSwip(Clock_Ip_SelectorConfigType const* Config)
{
    uint32 Instance;
    uint32 SelectorIndex;
    uint32 SelectorValue; /* Hw value corresponding to selector entry. Translate input clock source
                             to hardware value. */
    ...
    if (NULL_PTR != Config) {
        Instance = Clock_Ip_au8ClockFeatures[Config->Name][CLOCK_IP_MODULE_INSTANCE];
        SelectorIndex = Clock_Ip_au8ClockFeatures[Config->Name][CLOCK_IP_SELECTOR_INDEX];
        SelectorValue = Clock_Ip_au16SelectorEntryHardwareValue[Config->Value]; /* Hw value
                                                                                   corresponding to
                                                                                   selector entry.
                                                                                   Translate input
                                                                                   clock source to
        ...
        if (SelectorValue !=
            ((Clock_Ip_apxCgm[Instance][SelectorIndex]->CSS & SelectorMask) >> SelectorShift)) {
            Clock_Ip_StartTimeout(&StartTime, &ElapsedTime, &TimeoutTicks,
                CLOCK_IP_TIMEOUT_VALUE_US);
            do {
                TimeoutOccurred = Clock_Ip_TimeoutExpired(&StartTime, &ElapsedTime, TimeoutTicks);
            } while ((MC_CGM_MUX_CSS_SWIP_IN_PROGRESS ==
                         (Clock_Ip_apxCgm[Instance][SelectorIndex]->CSS &
                             MC_CGM_MUX_CSS_SWIP_MASK)) &&
                (FALSE == TimeoutOccurred));
            ...
            if (FALSE == TimeoutOccurred) {
                ...
                while ((CLOCK_IP_MC_CGM_MUX_CSS_CLK_SW_NOT_REQUESTED ==
                           (Clock_Ip_apxCgm[Instance][SelectorIndex]->CSS &
                               MC_CGM_MUX_CSS_CLK_SW_MASK)) &&
                    (FALSE == TimeoutOccurred));
                ...
                if (FALSE == TimeoutOccurred) {
                    ...
                    if (FALSE == TimeoutOccurred) {
                        ...
                        if (CLOCK_IP_MC_CGM_MUX_CSS_SWTRG_SUCCEEDED !=
                            ((Clock_Ip_apxCgm[Instance][SelectorIndex]->CSS &
                                 MC_CGM_MUX_CSS_SWTRG_MASK) >>
                                MC_CGM_MUX_0_CSS_SWTRG_SHIFT)) {
                            Clock_Ip_ReportClockErrors(CLOCK_IP_REPORT_CLOCK_MUX_SWITCH_ERROR,
                                Config->Name);
                        }
                    } else {
...
static void Clock_Ip_ResetCgmXCscCssCsGrip(Clock_Ip_SelectorConfigType const* Config)
{
    ...
    if (NULL_PTR != Config) {
        Instance = Clock_Ip_au8ClockFeatures[Config->Name][CLOCK_IP_MODULE_INSTANCE];
        SelectorIndex = Clock_Ip_au8ClockFeatures[Config->Name][CLOCK_IP_SELECTOR_INDEX];
        SelectorResetValue = Clock_Ip_au8SoftwareMuxResetValue[Config->Name]; /* Hw value
                                                                                 corresponding to
                                                                                 software mux reset.
                                                                               */
        ...
        Clock_Ip_apxCgm[Instance][SelectorIndex]->CSC |=
            (MC_CGM_MUX_CSC_CG_MASK | MC_CGM_MUX_CSC_FCG_MASK);

        Clock_Ip_StartTimeout(&StartTime, &ElapsedTime, &TimeoutTicks, CLOCK_IP_TIMEOUT_VALUE_US);
        do {
            TimeoutOccurred = Clock_Ip_TimeoutExpired(&StartTime, &ElapsedTime, TimeoutTicks);
        } while ((MC_CGM_MUX_CSS_CS_TRANSPARENT ==
                     (Clock_Ip_apxCgm[Instance][SelectorIndex]->CSS & MC_CGM_MUX_CSS_CS_MASK)) &&
            (FALSE == TimeoutOccurred));
        ...
        if (FALSE == TimeoutOccurred) {
            ...
            Clock_Ip_apxCgm[Instance][SelectorIndex]->CSC &=
                ~(MC_CGM_MUX_CSC_FCG_MASK | MC_CGM_MUX_CSC_CG_MASK);
        } else {
            /* Report timeout error */
            Clock_Ip_ReportClockErrors(CLOCK_IP_REPORT_TIMEOUT_ERROR, Config->Name);
        }
...
static void Clock_Ip_SetCgmXCscCssCsGrip(Clock_Ip_SelectorConfigType const* Config)
{
    ...
    if (NULL_PTR != Config) {
        ...
        if (FALSE == TimeoutOccurred) {
            ...
            do {
                TimeoutOccurred = Clock_Ip_TimeoutExpired(&StartTime, &ElapsedTime, TimeoutTicks);
            } while (((Clock_Ip_apxCgm[Instance][SelectorIndex]->CSS & MC_CGM_MUX_CSS_CS_MASK) !=
                         MC_CGM_MUX_CSS_CS_TRANSPARENT) &&
                (FALSE == TimeoutOccurred));
...
void Clock_Ip_ResetMcMeAeGssSysclk_TrustedCall(Clock_Ip_SelectorConfigType const* Config)
{
    ...
    if (NULL_PTR != Config) {
        ...
        Clock_Ip_apxSystemClock->POWER_MODE_CONFIG[PowerModeIndexIndex] &=
            ~MC_ME_AE_GS_S_SYSCLK_MASK;
    } else {
...
void Clock_Ip_SetMcMeAeGssSysclk_TrustedCall(Clock_Ip_SelectorConfigType const* Config)
{
    ...
    if (NULL_PTR != Config) {
        ...
        do {
            TimeoutOccurred = Clock_Ip_TimeoutExpired(&StartTime, &ElapsedTime, TimeoutTicks);
        } while ((CLOCK_IP_MC_ME_AE_TRANSITION_IS_ON_GOING ==
                     (IP_MC_ME_AE->GS & MC_ME_AE_GS_S_MTRANS_MASK)) &&
            (FALSE == TimeoutOccurred));
