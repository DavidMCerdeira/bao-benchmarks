...
static void Clock_Ip_CgmXPcfsSdurDivcDiveDivs(Clock_Ip_PcfsConfigType const* Config, uint32 CfgIndex)
{
    ...
    if (NULL_PTR != Config) {
        CgmPcfsBase = Clock_Ip_apxCgmPcfs[Clock_Ip_au8ClockFeatures[Config->SelectorName]
                                                                   [CLOCK_IP_MODULE_INSTANCE]];
        ...
        if (HashPfs[CfgIndex] !=
            (((uint32)Config->ClockSourceFrequency) ^ ((uint32)Config->MaxAllowableIDDchange) ^
                ((uint32)Config->Name) ^ ((uint32)Config->SelectorName) ^
                ((uint32)Config->StepDuration))) {
            HashPfs[CfgIndex] = (((uint32)Config->ClockSourceFrequency) ^
                ((uint32)Config->MaxAllowableIDDchange) ^ ((uint32)Config->Name) ^
                ((uint32)Config->SelectorName) ^ ((uint32)Config->StepDuration));

            Finput = Config->ClockSourceFrequency / CLOCK_IP_DIVIDE_BY_1000000;
#if defined(CLOCK_IP_HAS_FIRC_CLK)
            Fsafe = (*Clock_Ip_pxConfig->ConfiguredFrequencies)[Clock_Ip_FreqIds[FIRC_CLK]]
                        .ConfiguredFrequencyValue /
                CLOCK_IP_DIVIDE_BY_1000000;
#endif
#if (defined(CLOCK_IP_HAS_P5_AE_CLK) && defined(CLOCK_IP_HAS_FIRC_AE_CLK))
            if (P5_AE_CLK == Config->Name) {
                Fsafe = (*Clock_Ip_pxConfig->ConfiguredFrequencies)[Clock_Ip_FreqIds[FIRC_AE_CLK]]
                            .ConfiguredFrequencyValue /
                    CLOCK_IP_DIVIDE_BY_1000000;
            }
#endif
            ...
            AmaxBrut = (Config->MaxAllowableIDDchange * CLOCK_IP_DIVIDE_BY_1000000 /
                (Finput * CLOCK_IP_DYNAMIC_IDD_CHANGE));
            Rate = AmaxBrut;
            ...
            /* Calculate K by using formula k = ceil(0.5 + sqrt(0.25 - (2000 * (1 -(Finput/fsafe)) /
             * Rate))) */

            Var1 = 256U + ((CLOCK_IP_CONSTANT_2048000 * Finput) / (Fsafe * Rate)) -
                (CLOCK_IP_CONSTANT_2048000 / Rate);
            Var2 = 1UL << 30U; /* The second-to-top bit is set: use 1u << 14 for uint16 type; use
                                  1uL<<30 for uint32 type  */
            Var3 = 0U;
            ...
            K = (64U + 127U + (Var3 << 2U)) >> 7U; /* Calculated K from k = ceil(0.5 + sqrt(0.25 -
                                                      (2000 * (1 -(Fi/Fsafe)) / Rate))) */
            ...
            if (P5_AE_CLK == Config->Name) {
                ...
                IP_MC_CGM_AE->PCS_DIVC1 =
                    MC_CGM_PCS_DIVC1_RATE(DivcRate) | MC_CGM_PCS_DIVC1_INIT(DivcInit);
                ...
                CgmPcfsBase->PCFS[HwIndex].DIVC =
                    MC_CGM_PCFS_DIVC_RATE(DivcRate) | MC_CGM_PCFS_DIVC_INIT(DivcInit);
