...
#define CLOCK_IP_SW_PATCH_VERSION_C            1
...
static const Clock_Ip_ClockConfigType* Clock_Ip_pxConfig; /* Reference to the current clock
                                                             configuration */
#endif
...
#endif
        CLOCK_IP_DEV_ASSERT(
            (Clock_Ip_au8ClockNameTypes[(*(Config->Plls))[Config->PllsCount - 1U].Name] &
                CLOCK_IP_PLL_OBJECT) != 0U);
    }
}
...
#endif
        CLOCK_IP_DEV_ASSERT(
            (Clock_Ip_au8ClockNameTypes
                    [(*(Config->DividerTriggers))[Config->DividerTriggersCount - 1U].Name] &
                CLOCK_IP_DIVIDER_TRIGGER_OBJECT) != 0U);
    }
}
...
static void Clock_Ip_CheckPcfsClocks(Clock_Ip_ClockConfigType const* Config)
{
    ...
    if (Config->PcfsCount != 0U) {
        ...
        CLOCK_IP_DEV_ASSERT(
            (Clock_Ip_au8ClockNameTypes[(*(Config->Pcfs))[Config->PcfsCount - 1U].Name] &
                CLOCK_IP_PCFS_OBJECT) != 0U);
    }
}
...
static void Clock_Ip_CheckCmuClocks(Clock_Ip_ClockConfigType const* Config)
{
    ...
    if (Config->CmusCount != 0U) {
        ...
        CLOCK_IP_DEV_ASSERT(
            (Clock_Ip_au8ClockNameTypes[(*(Config->Cmus))[Config->CmusCount - 1U].Name] &
                CLOCK_IP_CMU_OBJECT) != 0U);
    }
}
#endif
...
static void Clock_Ip_ResetClockConfiguration(Clock_Ip_ClockConfigType const* Config)
{
    ...
    for (Index = Config->SelectorsCount; Index > 0U; Index--) /* Ramp down all selectors from
                                                                 configuration to SAFE_CLOCK */
    {
    ...
    for (Index = Config->FracDivsCount; Index > 0U; Index--) /* Put in reset state all fractional
                                                                dividers from configuration */
    {
    ...
    for (Index = Config->PllsCount; Index > 0U; Index--) /* Power down all plls from configuration
                                                          */
    {
        CallbackIndex = Clock_Ip_au8PllCallbackIndex
            [Clock_Ip_au8ClockFeatures[(*(Config->Plls))[Index - 1U].Name][CLOCK_IP_CALLBACK]];
    ...
    for (Index = Config->XoscsCount; Index > 0U; Index--) /* Power down all xoscs from configuration
                                                           */
    {
        CallbackIndex = Clock_Ip_au8XoscCallbackIndex
            [Clock_Ip_au8ClockFeatures[(*(Config->Xoscs))[Index - 1U].Name][CLOCK_IP_CALLBACK]];
...
void Clock_Ip_InitClock(Clock_Ip_ClockConfigType const* Config)
{
    ...
    if (NULL_PTR != Clock_Ip_pxConfig) {
        for (Index = 0U; Index < Config->CmusCount; Index++) /* Reset all clock monitor units from
    ...
    for (Index = 0U; Index < Config->IrcoscsCount; Index++) /* Set internal oscillators from
                                                               configuration */
    {
        CallbackIndex = Clock_Ip_au8IrcoscCallbackIndex
            [Clock_Ip_au8ClockFeatures[(*(Config->Ircoscs))[Index].Name][CLOCK_IP_CALLBACK]];
    ...
    /* Configure the PCFS  */
    for (Index = 0U; Index < Config->PcfsCount; Index++) /* Configure all progressive frequency
                                                            switching clocks from configuration */
    {
        CallbackIndex =
    ...
    for (Index = 0U; Index < Config->DividerTriggersCount; Index++) /* Set divider triggers from
                                                                       configuration. */
    {
    ...
    {
        CallbackIndex = Clock_Ip_au8DividerCallbackIndex
            [Clock_Ip_au8ClockFeatures[(*(Config->Dividers))[Index].Name][CLOCK_IP_CALLBACK]];
    ...
    for (Index = 0U; Index < Config->PllsCount; Index++) /* Configure all plls from configuration
                                                            asynchronously. Do not enable. */
    {
        CallbackIndex =
    ...
    for (Index = 0U; Index < Config->CmusCount; Index++) /* Set the Clock Monitoring Units that are
                                                            under mcu control. Cmus are not enabled.
                                                          */
    {
        CallbackIndex =
            Clock_Ip_au8CmuCallbackIndex[Clock_Ip_au8ClockFeatures[(*(Config->Cmus))[Index].Name]
    ...
    for (Index = 0U; Index < Config->XoscsCount; Index++) /* Wait for all xoscs from configuration
                                                             to lock */
    {
        CallbackIndex =
            Clock_Ip_au8XoscCallbackIndex[Clock_Ip_au8ClockFeatures[(*(Config->Xoscs))[Index].Name]
    ...
    /* Configure PLL clock generators */
    for (Index = 0U; Index < Config->PllsCount; Index++) /* Enable plls according to configuration
    ...
    for (Index = 0U; Index < Config->FracDivsCount; Index++) /* Configure all fractional dividers
                                                                from configuration asynchronously.
                                                                Do not wait. */
    {
        CallbackIndex = Clock_Ip_au8FractionalDividerCallbackIndex
     ...
     * stabilized already. */
    for (Index = 0U; Index < Config->SelectorsCount; Index++) /* Set only if selected inputs are not
                                                                 clocked from PLLs */
    {
    ...
    if (FALSE == DriverContext.ClockTreeIsConsumingPll) {
        for (Index = 0U; Index < Config->GatesCount; Index++) /* Set clock gates that are under
                                                                 clock control. */
        {
            CallbackIndex = Clock_Ip_au8GateCallbackIndex
                [Clock_Ip_au8ClockFeatures[(*(Config->Gates))[Index].Name][CLOCK_IP_CALLBACK]];
...
void Clock_Ip_DistributePll(void)
{
    ...
    if (NULL_PTR != Clock_Ip_pxConfig) {
        for (Index = 0U; Index < Clock_Ip_pxConfig->SelectorsCount; Index++) /* Set only if selected
                                                                                inputs are clocked
                                                                                from PLLs */
        {
            if (PLL_TYPE ==
                Clock_Ip_aeSourceTypeClockName[(*(Clock_Ip_pxConfig->Selectors))[Index].Value]) {
                CallbackIndex = Clock_Ip_au8SelectorCallbackIndex[Clock_Ip_au8ClockFeatures
                        [(*(Clock_Ip_pxConfig->Selectors))[Index].Name][CLOCK_IP_CALLBACK]];
        ...
        /* Check if the clock tree is using a PLL output */
        if (DriverContext.ClockTreeIsConsumingPll) {
            for (Index = 0U; Index < Clock_Ip_pxConfig->GatesCount; Index++) /* Set clock gates that
                                                                                are under clock
                                                                                control. */
            {
                CallbackIndex = Clock_Ip_au8GateCallbackIndex[Clock_Ip_au8ClockFeatures
                        [(*(Clock_Ip_pxConfig->Gates))[Index].Name][CLOCK_IP_CALLBACK]];
                Clock_Ip_axGateCallbacks[CallbackIndex].Set(&(*Clock_Ip_pxConfig->Gates)[Index]);
            }

            /* Enable the Clock Monitoring Units ( CMU0 .. n ) according to configuration. */
            for (Index = 0U; Index < Clock_Ip_pxConfig->CmusCount; Index++) {
                CallbackIndex = Clock_Ip_au8CmuCallbackIndex[Clock_Ip_au8ClockFeatures
                        [(*(Clock_Ip_pxConfig->Cmus))[Index].Name][CLOCK_IP_CALLBACK]];
