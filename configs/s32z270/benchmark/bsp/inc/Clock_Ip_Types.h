...
typedef struct {
    ...
    uint64 Value; /**< Enable value - if value is zero then clock is gated, otherwise is enabled in
                     different modes. */
 ...
 */
typedef struct {
    Clock_Ip_NameType Name;         /**<  Clock source from which ramp-down and to which ramp-up are
                                       processed. */
...
typedef struct {
    ...
    const Clock_Ip_ConfiguredFrequencyType (*ConfiguredFrequencies)[]; /**< Configured frequency
                                                                          values */
