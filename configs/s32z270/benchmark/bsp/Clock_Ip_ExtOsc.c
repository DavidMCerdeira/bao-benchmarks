...
#define CLOCK_IP_EXTOSC_SW_PATCH_VERSION_C            1
...
static void Clock_Ip_ResetFxoscOsconBypEocvGmSel(Clock_Ip_XoscConfigType const* Config)
{
    ...
    if (NULL_PTR != Config) {
        ...
        do {
            FxoscStatus = ((Clock_Ip_apxXosc[Instance]->STAT & FXOSC_STAT_OSC_STAT_MASK) >>
                FXOSC_STAT_OSC_STAT_SHIFT);
...
static void Clock_Ip_SetFxoscOsconBypEocvGmSel(Clock_Ip_XoscConfigType const* Config)
{
    ...
    if (NULL_PTR != Config) {
        ...
        if (Config->Enable != 0U) {
            Clock_Ip_apxXosc[Instance]->CTRL = (FXOSC_CTRL_OSCON(1U) |
                FXOSC_CTRL_OSC_BYP(Config->BypassOption) | FXOSC_CTRL_EOCV(Config->StartupDelay) |
                FXOSC_CTRL_GM_SEL(Config->TransConductance) | FXOSC_CTRL_COMP_EN(Config->CompEn) |
...
static void Clock_Ip_CompleteFxoscOsconBypEocvGmSel(Clock_Ip_XoscConfigType const* Config)
{
    ...
    if (NULL_PTR != Config) {
        ...
        if (Config->Enable != 0U) {
            /* This assumes that FXOSC_CTRL[COMP_EN] = 1 and FXOSC_CTRL[OSC_BYP] = 0 (i.e. crystal
             * clock/oscillator output). */
            /* In bypass mode (i.e. EXTAL output), FXOSC_CTRL[COMP_EN] = 0 and FXOSC_CTRL[OSC_BYP] =
               1, which means that we cannot check for stabilization. This shifts the responsibility
               of waiting for a stable clock to the upper layers. */
            if (Config->CompEn != 0U) {
                if (0U == Config->BypassOption) {
                    ...
                    do {
                        FxoscStatus =
                            ((Clock_Ip_apxXosc[Instance]->STAT & FXOSC_STAT_OSC_STAT_MASK) >>
                                FXOSC_STAT_OSC_STAT_SHIFT);
                        TimeoutOccurred =
                    ...
                    /* Invalid FXOSC configuration: FXOSC_CTRL[COMP_EN] = 1 enforces
                     * FXOSC_CTRL[OSC_BYP] = 0. */
                    /* Report timeout error */
                    Clock_Ip_ReportClockErrors(CLOCK_IP_REPORT_FXOSC_CONFIGURATION_ERROR,
                        Config->Name);
                }
            }
        }
