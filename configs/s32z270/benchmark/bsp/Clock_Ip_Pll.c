...
static void Clock_Ip_ResetPlldigRdivMfiMfnSdmenSsscgbypSpreadctlStepnoStepsize(
    Clock_Ip_PllConfigType const* Config);
static void Clock_Ip_SetPlldigRdivMfiMfnSdmenSsscgbypSpreadctlStepnoStepsize(
    Clock_Ip_PllConfigType const* Config);
static Clock_Ip_PllStatusReturnType
Clock_Ip_CompletePlldigRdivMfiMfnSdmenSsscgbypSpreadctlStepnoStepsize(Clock_Ip_NameType PllName);
static void Clock_Ip_EnablePlldigRdivMfiMfnSdmenSsscgbypSpreadctlStepnoStepsize(
    Clock_Ip_PllConfigType const* Config);
#endif
        ...
        if (Config->Enable != 0U) {
            ...
            switch (Config->InputReference) {
                case FIRC_CLK:
                    ...
                    Clock_Ip_apxPll[Instance].PllInstance->PLLCLKMUX =
                        PLLDIG_PLLCLKMUX_REFCLKSEL(CLOCK_IP_FIRC_PLL_REFERENCE);
                    break;
                case FXOSC_CLK:
                    /* Select input reference. */
                    Clock_Ip_apxPll[Instance].PllInstance->PLLCLKMUX =
                        PLLDIG_PLLCLKMUX_REFCLKSEL(CLOCK_IP_FXOSC_PLL_REFERENCE);
                    break;
                default:
...
Clock_Ip_CompletePlldigRdivMfiMfnSdmenSsscgbypSpreadctlStepnoStepsize(Clock_Ip_NameType PllName)
{
    ...
    if (0U == (Clock_Ip_apxPll[Instance].PllInstance->PLLCR & PLLDIG_PLLCR_PLLPD_MASK)) {
        ...
        do {
            PllLockStatus =
                ((Clock_Ip_apxPll[Instance].PllInstance->PLLSR & PLLDIG_PLLSR_LOCK_MASK) >>
                    PLLDIG_PLLSR_LOCK_SHIFT);
...
static void Clock_Ip_SetLfastPLL(Clock_Ip_PllConfigType const* Config)
{
    ...
    if (NULL_PTR != Config) {
        ...
        if (1U == Config->Enable) {
            ...
            Clock_Ip_apxLfastPll[Instance].PllInstance->MCR &= (~((uint32)LFAST_MCR_DRFEN_MASK));
#ifndef ERR_IPV_LFAST_PLL_051380
            Clock_Ip_apxLfastPll[Instance].PllInstance->PLLCR &=
                (~((uint32)LFAST_PLLCR_FBDIV_MASK));
            Clock_Ip_apxLfastPll[Instance].PllInstance->PLLCR |=
                LFAST_PLLCR_FBDIV((uint8)(Config->MulFactorDiv));

            Clock_Ip_apxLfastPll[Instance].PllInstance->PLLCR &=
                (~((uint32)LFAST_PLLCR_PREDIV_MASK));
            Clock_Ip_apxLfastPll[Instance].PllInstance->PLLCR |=
                LFAST_PLLCR_PREDIV((uint8)(Config->Predivider - 1U));

            Clock_Ip_apxLfastPll[Instance].PllInstance->PLLCR &=
                (~((uint32)LFAST_PLLCR_FDIVEN_MASK));
            Clock_Ip_apxLfastPll[Instance].PllInstance->PLLCR |=
                LFAST_PLLCR_FDIVEN(Config->SigmaDelta);
#else
            /* 3. Turn on the PLL  */
...
static Clock_Ip_PllStatusReturnType Clock_Ip_CompleteLfastPLL(Clock_Ip_NameType PllName)
{
    ...
    uint32 Instance = Clock_Ip_au8ClockFeatures[PllName][CLOCK_IP_MODULE_INSTANCE];
    if (1U ==
        ((Clock_Ip_apxLfastPll[Instance].PllInstance->PLLCR & LFAST_PLLCR_SWPON_MASK) >>
            LFAST_PLLCR_SWPON_SHIFT)) {
        Clock_Ip_StartTimeout(&StartTime, &ElapsedTime, &TimeoutTicks, CLOCK_IP_TIMEOUT_VALUE_US);
        /* Wait until this pll is enable */
        do {
            PllEnableStatus =
                ((Clock_Ip_apxLfastPll[Instance].PllInstance->PLLLSR & LFAST_PLLLSR_PLLDIS_MASK) >>
                    LFAST_PLLLSR_PLLDIS_SHIFT);
            TimeoutOccurred = Clock_Ip_TimeoutExpired(&StartTime, &ElapsedTime, TimeoutTicks);
        ...
        if (TRUE == TimeoutOccurred) {
            ...
            /* Wait until this pll is locked */
            do {
                PllLockStatus = ((Clock_Ip_apxLfastPll[Instance].PllInstance->PLLLSR &
                                     LFAST_PLLLSR_PLDCR_MASK) >>
                    LFAST_PLLLSR_PLDCR_SHIFT);
                TimeoutOccurred = Clock_Ip_TimeoutExpired(&StartTime, &ElapsedTime, TimeoutTicks);
...
static void Clock_Ip_EnableLfastPLL(Clock_Ip_PllConfigType const* Config)
{
    ...
    if (NULL_PTR != Config) {
        ...
        if (1U == Config->Enable) {
            ...
            Clock_Ip_apxLfastPll[Instance].PllInstance->PLLCR &=
                ~(CLOCK_IP_LFASTPLL_PLLCR_RESERVED_RWBIT_MASK);
