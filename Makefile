...
ctx-switch:
	make -C src/ctx-switch bmrt_dir=$(bmrt_dir) \
		PLATFORM=$(PLATFORM) \
...
irq-lat:
	@echo GUEST1_ARGS=${GUEST1_ARGS} NATIVE_ARGS=$(GUEST2_ARGS)
	make -C src/irq-lat bmrt_dir=$(bmrt_dir) \
		PLATFORM=$(PLATFORM) \
...
irq-lat-wce:
	make -C src/irq-lat-wce bmrt_dir=$(bmrt_dir) \
		PLATFORM=$(PLATFORM) \
		GUEST1_ARGS=$(GUEST1_ARGS) GUEST2_ARGS=$(GUEST2_ARGS)
